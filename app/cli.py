...
def main():
    try:
        ...
        sub_index = (
            click.prompt(
                click.style("Enter the number of the subscription to use", fg="green"),
                type=int,
            )
            - 1
        )
        ...
        operation = click.prompt(
            click.style(
                "Enter the number of the operation you want to perform", fg="green"
            ),
            type=int,
        )
        ...
        if operation == 1:  # Enroll VM
            ...
            rg_index = (
                click.prompt(
                    click.style(
                        "Enter the number of the resource group to list VMs", fg="green"
                    ),
                    type=int,
                )
                - 1
            ...
            if vms:
                ...
                vm_index = (
                    click.prompt(
                        click.style(
                            "Enter the number of the VM to proceed", fg="green"
                        ),
                        type=int,
                    )
                    - 1
                ...
                vm_password = click.prompt(
                    click.style("Enter VM password", fg="green"), hide_input=True
                )
                ...
                if status == "not_found":
                    click.echo(
                        f"Elastic Agent not found on VM {vm_name}. Proceeding with installation."
                    )
                    copy_http_ca_to_vm(
                    ...
                    click.echo(
                        f"Elastic Agent is inactive on VM {vm_name}. Restarting."
                    )
                    restart_elastic_agent(ssh_client)
                elif status == "active":
                    click.echo(
                        f"Elastic Agent is already running on VM {vm_name}. VM is being monitored."
                    )
                else:
            ...
            if vms:
                ...
                vm_index = (
                    click.prompt(
                        click.style(
                            "Enter the number of the VM to unenroll", fg="green"
                        ),
                        type=int,
                    )
                    - 1

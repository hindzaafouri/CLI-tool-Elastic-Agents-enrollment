...
def copy_http_ca_to_vm(vm_ip, os_type, storage_account_name, storage_account_key, vm_username, vm_password):
    try:
        ...
        with Halo(text=click.style('Downloading http_ca.crt...', fg='green'), spinner='dots'):
            with open(local_path, "wb") as file:
                download_stream = blob_client.download_blob()
                file.write(download_stream.readall())
            click.echo(f"File downloaded successfully to {local_path}")

        with Halo(text=click.style('Copying http_ca.crt to VM...', fg='green'), spinner='dots'):
...
def install_elastic_agent(ssh_client, username):
    try:
        ...
        with Halo(text=click.style('Installing Elastic Agent...', fg='green'), spinner='dots'):
...
def restart_elastic_agent(ssh_client):
    try:
        ...
        with Halo(text=click.style('Restarting Elastic Agent...', fg='green'), spinner='dots'):
            stdin, stdout, stderr = ssh_client.exec_command(command)
            stdout.channel.recv_exit_status()
            output = stdout.read().decode('utf-8')
            error = stderr.read().decode('utf-8')
